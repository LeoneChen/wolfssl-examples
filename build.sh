...
WOLFSSL_MAKE_FLAGS="CC=clang-13 SGX_SDK=${SGXSAN_DIR} HAVE_WOLFSSL_TEST=1 HAVE_WOLFSSL_BENCHMARK=1 HAVE_WOLFSSL_SP=1 SGX_MODE=SIM -j$(nproc)"
EXAMPLE_MAKE_FLAGS="CC=clang-13 CXX=clang++-13 LD=lld SGX_SDK=${SGXSAN_DIR} SGX_WOLFSSL_LIB=${ROOT_DIR}/wolfssl/IDE/LINUX-SGX/ WOLFSSL_ROOT=${ROOT_DIR}/wolfssl/ HAVE_WOLFSSL_TEST=1 HAVE_WOLFSSL_BENCHMARK=1 HAVE_WOLFSSL_SP=1 SGX_MODE=SIM -j$(nproc)"
...
then
    ...
    WOLFSSL_CFLAGS+=" -fsanitize-coverage=inline-8bit-counters,bb,no-prune,pc-table,trace-cmp -fprofile-instr-generate -fcoverage-mapping"
fi
